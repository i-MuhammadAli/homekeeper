...
import { deletePropertyDoc } from "../../../store";
import createApi from "../../../utils/api";
// import { handleFileDownload } from "../property/PropertySection";

function DocumentSection({ parentClass, fileNameSize, fileNameTextLimit }) {
  const api = createApi();
            ...
            <div key={item.uuid}>
              <div className="flex items-center flex-row justify-between">
                <div className="flex flex-row items-center">
                  ...
                  />
                  <div
                    className={`info_style cursor-pointer hover:underline ${fileNameSize}`}
                    onClick={() =>
                      handleFileDownload(item.uuid, item.name, item.extension)
                    }
                  >
                    {truncateText(item.name, fileNameTextLimit)}
                  </div>
                </div>

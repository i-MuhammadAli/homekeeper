...
function Homeinspection() {
  ...
  const columns = [
    ...
    {
      name: "Link to maintainance",
      selector: (row) => row.linktomaintainance,
      sortable: true,
    },
  ];
  ...
  const sendDetailsToServer = async () => {
    axios
      ...
      .then(function (response) {
        console.log("response", response.data.inspectionrequesttoadmin);
        // Setdata(response.data.getplans);
        // this.setState({data: data.conversations});
        if (response.data.status == "success") {
          const maintainencetemplate = response.data.inspectionrequesttoadmin;
          let data = [];
          for (let index = 0; index < maintainencetemplate.length; index++) {
            data.push({
              id: index + 1,
              username: maintainencetemplate[index].fname,
              usertype: maintainencetemplate[index].agentname,
              homeownername: maintainencetemplate[index].fname,
              amountpaid: "$" + maintainencetemplate[index].homevalue,
              servicetype: (
                <Link
              ...
              daterequested: maintainencetemplate[index].whenbuy,
              linktomaintainance: (
                <Link
                  to={"#" + maintainencetemplate[index].id}
                  onClick={() => {
                    startplan(maintainencetemplate[index].id);
                  }}
                  className="strthomeplan"
                >
                  Start Home Maintainenace plan
                </Link>
              ),
              // templatename: maintainencetemplate[index].templatename,
              // action: <><i className="fa-solid fa-pen" data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={() => { handleSubmitClick(maintainencetemplate[index].id, maintainencetemplate[index].templatename) }}></i><i className="fa-regular fa-trash-can" onClick={() => { handledeleteClick(maintainencetemplate[index].id, maintainencetemplate[index].templatename) }}></i></>
            });
          }
      ...
      <section className="dashboard">
        <div className="container-fluid">
          ...
          <div className="inspection_reques maintenance">
            ...
            <div className="table-responsive w-100 allact">
